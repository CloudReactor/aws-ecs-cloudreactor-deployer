...
want to create a new image based on `cloudreactor/aws-ecs-cloudreactor-deployer`:
        ...
        FROM cloudreactor/aws-ecs-cloudreactor-deployer:4.1.3
...
| DOCKER_IMAGE              	|`ghcr.io/cloudreactor/aws-ecs-cloudreactor-deployer`	| The Docker image to run. You can set this to `public.ecr.aws/cloudreactor/aws_ecs_cloudreactor_deployer` if deploying from within AWS, to get the image from AWS ECR. You can also set this to another name in case you extend the image to add build or deployment tools.	|
| DOCKER_IMAGE_TAG           	|`4`	| The tag of the Docker image to run. Can also be set to pinned versions like `4.1.3`, compatible releases like `4.1`, or `latest`. |
...
the master branch:
    ...
    jobs:
      deploy:
        ...
        - name: Deploy to AWS ECS and CloudReactor
          uses: CloudReactor/aws-ecs-cloudreactor-deployer@v4.1.3
          with:
