...
want to create a new image based on `cloudreactor/aws-ecs-cloudreactor-deployer`:
        ...
        FROM cloudreactor/aws-ecs-cloudreactor-deployer:4.2.2
...
| DOCKER_IMAGE_TAG           	|`4`	| The tag of the Docker image to run. Can also be set to pinned versions like `4.2.2`, compatible releases like `4.2`, or `latest`. |
...
the master branch:
    ...
    jobs:
      deploy:
        ...
        - name: Deploy to AWS ECS and CloudReactor
          uses: CloudReactor/aws-ecs-cloudreactor-deployer@v4.2.2
          with:
