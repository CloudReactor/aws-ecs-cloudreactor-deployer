...
want to create a new image based on `cloudreactor/aws-ecs-cloudreactor-deployer`:
        ...
        FROM cloudreactor/aws-ecs-cloudreactor-deployer:3.2.1
...
| DOCKER_IMAGE_TAG           	|`3`	| The tag of the Docker image to run. Can also be set to pinned versions like `3.2.1`, compatible releases like `3.2`, or `latest`. |
...
the master branch:
    ...
    jobs:
      deploy:
        ...
        - name: Deploy to AWS ECS and CloudReactor
          uses: CloudReactor/aws-ecs-cloudreactor-deployer@v3.2.1
          with:
